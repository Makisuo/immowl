...
export const getOwnerProperties = query({
	...
	handler: async (ctx, args) => {
		...
		let query = ctx.db.query("properties").withIndex("by_owner", (q) => q.eq("ownerId", user._id))
...
export const getOwnerPropertyAnalytics = query({
	args: {
		timeframe: v.optional(v.union(v.literal("week"), v.literal("month"), v.literal("year"))),
	},
	returns: v.object({
